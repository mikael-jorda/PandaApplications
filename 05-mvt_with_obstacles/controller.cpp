...
const string robot_file = "../resources/05-mvt_with_obstacles/panda_arm.urdf";
...
int main() {
	...
	while (runloop) {
		...
		else if(state == HOLD_CARTESIAN_POS)
		{
			...
			if((controller_counter - state_init_counter) % 20000 == 10000)
			{
				pos_task->_desired_position(1) -= 0.2;
			}
			else if((controller_counter - state_init_counter) % 20000 == 0)
			{
				pos_task->_desired_position(1) += 0.2;
			}
