...
int main() {
	...
	for(int i=0 ; i<n_robots ; i++)
	{
		...
		posori_tasks[i]->_kv_pos = 22.0;
		posori_tasks[i]->_kp_ori = 500.0;
		posori_tasks[i]->_kv_ori = 22.0;		
	...
	allegro_init_config << 0.0671049,0.188217,0.163665,0.123758,0.0626668,0.10281,0.144723,0.109697,0.0387895,0.164198,0.142931,0.111295,-1.22342,0.117256,0.189332,0.268245;
	...
	while (runloop) {
		...
		else if(state == WAIT_FOR_CAMERA_AND_IK)
		{
			...
			if(redis_client.get(IK_FINISHED_KEY) == "1")
			{
				...
				// posori_tasks[0]->_desired_position += 0.05 * Vector3d::UnitZ();
		...
		vector<sf::safe_ptr<Sai2Primitives::PosOriTask>> posori_tasks)
{
	...
	while(runloop)
	{
		...
		else
		{
			...
			for(int j=4 ; j<7 ; j++)
			{
				robots[1]->_M(j,j) += 0.15;
			}
		
			for(int j=3 ; j<6 ; j++)
			{
				posori_tasks[0]->_Lambda(j,j) += 0.15;
			}
