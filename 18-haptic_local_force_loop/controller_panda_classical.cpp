...
int main() {
	...
	while (runloop)
	{
		...
		sensed_force_global = sensed_force_moment_world_frame.head(3);
		...
		else if(state == CONTROL)
		{
			...
			// teleop_task->_commanded_force_device -= kv_haptic * teleop_task->_current_trans_velocity_device;
			...
			double vh_square = teleop_task->_current_trans_velocity_device.squaredNorm();
			double alpha_pc = 0;
			if(haptic_PO > 0 && vh_square > 0.0001)
			{
				alpha_pc = haptic_PO / vh_square * 1000.0;
			}
			if(alpha_pc > 0.9 * _max_damping_device0[0])
			{
				alpha_pc = 0.9 * _max_damping_device0[0];
			}
			if(alpha_pc < 0)
			{
				alpha_pc = 0;
			...
			teleop_task->_commanded_force_device -= alpha_pc * teleop_task->_current_trans_velocity_device;
			haptic_PO -= alpha_pc * vh_square * 0.001;

			// cout << "PO haptic : " << haptic_PO << endl;
			// cout << "alpha : " << alpha_pc << endl;
			// cout << endl;
