...
void control(Sai2Model::Sai2Model* robot, Simulation::Sai2Simulation* sim)
{
	...
	// shoulder task
	MatrixXd J_shoulder_base = MatrixXd::Zero(1,dof);
	J_shoulder_base(0,0) = 1;
	MatrixXd J_shoulder = MatrixXd::Zero(1,dof);
	MatrixXd Jbar_shoulder = MatrixXd::Zero(dof,1);
	MatrixXd Lambda_shoulder = MatrixXd::Zero(1,1);
	MatrixXd N_shoulder = MatrixXd::Zero(dof,dof);
	double shoulder_init = robot->_q(0);

	double kp_shoulder = 100.0;
	double kv_shoulder = 20.0;

	VectorXd shoulder_task_torques;
	...
	// logger
	string folder = "../../20-bracing_one_arm_floating_base/data_logging/data/";
	...
	while (fSimulationRunning) 
	{
		...
		posori_task_range_space = MatrixXd::Identity(dof,dof) - posori_task->_N;
		N_prec = posori_task->_N;

		J_shoulder = J_shoulder_base * N_prec;
		robot->operationalSpaceMatrices(Lambda_shoulder, Jbar_shoulder, N_shoulder, J_shoulder, N_prec);
		N_prec = N_shoulder;
		...
		posori_task_torques += posori_task_range_space.transpose() * gravity;
		posori_task_torques += posori_task_range_space.transpose() * tau_contact_observed;

		// shoulder
		VectorXd shoulder_task_force = Lambda_shoulder * (-kp_shoulder * (robot->_q(0) - shoulder_init) - kv_shoulder * robot->_dq(0));
		shoulder_task_force += Jbar_shoulder.transpose() * (gravity + tau_contact_observed);

		shoulder_task_torques = J_shoulder.transpose() * shoulder_task_force;

		// joint task
		joint_task->computeTorques(joint_task_torques);
		if(controller_counter < 15000)
		{
		...
		// alpha = -0.3;
		bracing_task_torques = alpha * range_space_bracing.transpose() * gravity;

		// final torques
		command_torques = posori_task_torques + bracing_task_torques + joint_task_torques + shoulder_task_torques;
